...
public class ChunkedChildren {
	...
	public static void main(String args[]) throws IOException {
		for (int i = 0; i < id.length; i++) {
			...
			Hashtable<Long, LinkedList<Child>> chunk = new Hashtable<Long, LinkedList<Child>>();
			String in;
			...
			while (e.hasMoreElements()) {
				...
				children = uniquify(children); //to ensure that the child list is unique
	...
	private static LinkedList<Child> uniquify(LinkedList<Child> children) {
		Collections.sort(children);
		LinkedList<Child> ans = new LinkedList<Child>();
		ans.add(children.get(0));
		for (int i = 1; i < children.size(); i++)
			if (children.get(i).id != children.get(i - 1).id)
